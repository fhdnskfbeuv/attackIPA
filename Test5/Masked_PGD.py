...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn,
                      ...
                      l1_sparsity=None, mask=None, verbose=False, eot=1):
    """
    ...
    for ii in range(nb_iter):
        for eotI in range(eot):
            ...
            loss.backward()
        if verbose:
            print('\riter{}: loss:{} grad:{}'.format(ii,
                                                     loss.item(),
                                                     torch.mean(torch.linalg.norm(delta.grad.data.view(delta.grad.shape[0], -1), dim=-1)).to(torch.float32).item()),
                  end='')
        if ord == np.inf:
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None, mask=None, verbose=False, eot=1):
        """
        ...
        rval = perturb_iterative(
            ...
            l1_sparsity=self.l1_sparsity, mask=mask, verbose=verbose, eot=eot
        )
